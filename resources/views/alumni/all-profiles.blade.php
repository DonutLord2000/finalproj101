                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-9" id="profiles-container">
                        @foreach($profiles as $profile)
                            <a href="{{ route('alumni.profile.show', $profile->id) }}"
                                ...
                                <img src="{{ $profile->profile?->profile_picture ? Storage::url($profile->profile->profile_picture) : asset('storage/profile-photos/default.png') }}"
                                     alt="{{ $profile->name }}"
                                ...
                                <p class="text-gray-600 text-sm text-center">
                                    {{ $profile->profile?->address ?? 'Location not specified' }}
                                </p>
